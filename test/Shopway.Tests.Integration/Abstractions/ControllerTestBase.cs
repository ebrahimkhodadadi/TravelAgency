...
using TravelAgency.Tests.Integration.Utilities;
...
namespace TravelAgency.Tests.Integration.Abstractions;

public abstract class ControllerTestsBase : IDisposable
{
    private readonly string TravelAgencyApiUrl;
    ...
    public ControllerTestsBase(DependencyInjectionContainerTestFixture containerTestFixture)
    {
        ...
        TravelAgencyApiUrl = integrationTestsUrlOptions.TravelAgencyApiUrl!;
        _controllerUri = GetType().Name[..^ControllerTests.Length];

        var restClientOptions = new RestClientOptions(new Uri($"{TravelAgencyApiUrl}{nameof(UsersController)[..^Controller.Length]}"))
        {
    ...
    protected async Task<RestClient> RestClient(string controllerUrl, DatabaseFixture databaseFixture)
    {
        ...
        var restClientOptions = new RestClientOptions(new Uri($"{TravelAgencyApiUrl}{controllerUrl}"))
        {
